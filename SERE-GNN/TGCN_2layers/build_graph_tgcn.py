...
# build corpus
dataset = 'ptc'#DataSet_Misinfo,proppysm
...
##word_vector_file = 'D:/ProgramData/glove.6B.300d.txt'
...
f = open(input1 + '_labels.txt', 'r', encoding='latin1')
lines = f.readlines()
for line in lines:
...
# x: feature vectors of train docs, no initial features
# slect 90% train set
...
# test_ids_str = '\n'.join(str(i+vocab_size+train_size) for i in range(0,len(test_ids)))
# # test_ids_str = '\n'.join(str(int(index)-int(len(test_ids))+vocab_size+train_size) for index in test_ids)
# f = open(output1 + '.test.index', 'w',encoding='utf-8')#存放test数据集的序号
# f.write(test_ids_str)
# f.close()

# different train rates
...
# allx: the the feature vectors of both labeled and unlabeled train instances
# (a superset of x)
# unlabeled train instances -> words
...
#计算词汇表中词向量之间的相似度，并找出相似度大于0.9的词对
'''
for i in range(vocab_size):
